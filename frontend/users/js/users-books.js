...
async function fetchBorrowedBooks() {
    try {
        ...
        if (response.ok) {
            ...
            // Assuming bookList is your list element
            if (borrowedBooks.length === 0) {
                const noRecordsItem = document.createElement('li');
                noRecordsItem.classList.add('list-group-item', 'text-muted'); // Add appropriate classes for styling
                noRecordsItem.textContent = 'No records found.';
                bookList.appendChild(noRecordsItem);
            } else {
